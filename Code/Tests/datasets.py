...
script_dir = os.path.dirname(__file__)  # 用于获取当前脚本文件所在的目录路径
other_dir = os.path.join(script_dir, "..", "Other")   # 将当前目录的上级目录与子目录名 "Other" 连接起来，形成完整的目录路径
sys.path.append(other_dir)  # 将新的目录路径添加到 Python 解释器的搜索路径中，这样 Python 就可以在运行时找到该目录下的模块或文件
from utility_functions import make_coord_grid, normal, nc_to_tensor, curl, tensor_to_cdf
# from Code.Other.utility_functions import make_coord_grid, normal, nc_to_tensor, curl, tensor_to_cdf
# print(normal)
import torch
...
project_folder_path = os.path.dirname(os.path.abspath(__file__))   # 用于获取当前脚本文件的绝对路径
project_folder_path = os.path.join(project_folder_path, "..", "..")  # 用于获取当前脚本文件所在的目录路径
data_folder = os.path.join(project_folder_path, "Data")    # 用于将各个目录名连接成完整的路径
